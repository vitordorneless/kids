...
class Alergia extends Magica {
    ...
    public function save($tipo, $user_created, $date_created, $user_edit, $date_edit) {
        ...
        $smtp = $pdo->prepare("INSERT INTO alergia(tipo,user_created,date_created,user_edit,date_edit,status,data_ultima_alteracao) VALUES(?,?,?,?,?,?,?)");
    ...
    public function edit($id, $tipo, $user_created, $date_created, $user_edit, $date_edit,$status) {
        ...
        $data_ultima_alteracao = date('Y-m-d H:i:s');
        $pdo = Database::connect();
        $smtp = $pdo->prepare("UPDATE alergia SET tipo = ?,user_created = ?,date_created = ?,user_edit = ?,date_edit = ?,status = ?,data_ultima_alteracao = ? WHERE id = ?");
        $smtp->bindParam(1, $tipo, PDO::PARAM_STR);
        $smtp->bindParam(2, $user_created, PDO::PARAM_INT);
        $smtp->bindParam(3, $date_created, PDO::PARAM_STR);
        $smtp->bindParam(4, $user_edit, PDO::PARAM_INT);
    ...
    public function inativar($id) {
        ...
        $data_ultima_alteracao = date('Y-m-d H:i:s');
        $pdo = Database::connect();
        $smtp = $pdo->prepare("UPDATE alergia SET status = 0,data_ultima_alteracao = ? WHERE id = ?");
    ...
    public function Dados($id) {
        ...
        $sql = "select id,tipo,user_created,date_created,user_edit,date_edit,status,data_ultima_alteracao from alergia where id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
